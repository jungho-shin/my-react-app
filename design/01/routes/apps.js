...
/* GET /datacollector/countries page. */
router.get('/datacollector/countries', function(req, res, next) {
  ...
  db.query(query, [], function (error, results, fields) {
    ...
    res.render('apps/datacollector/countries', { title: 'Express', countries: results });
  
  });

});

/* GET /datacollector/datatypes page. */
router.get('/datacollector/datatypes', function(req, res, next) {
  ...
  db.query(query, [], function (error, results, fields) {
    ...
    res.render('apps/datacollector/datatypes', { title: 'Express', datatypes: results });
  
  });
});

/* GET /datacollector/timeunits page. */
router.get('/datacollector/timeunits', function(req, res, next) {
  ...
  db.query(query, [], function (error, results, fields) {
    ...
    res.render('apps/datacollector/timeunits', { title: 'Express', timeunits: results });
  
  });
});

/* GET /datacollector/taskstatus page. */
router.get('/datacollector/taskstatus', function(req, res, next) {
  ...
  db.query(query, [], function (error, results, fields) {
    ...
    res.render('apps/datacollector/taskstatus', { title: 'Express', taskstatus: results });
  
  });
});

/* GET /datacollector/schedules page. */
router.get('/datacollector/schedules', function(req, res, next) {

  var query1 = `SELECT * FROM datatypes WHERE delete_date IS NULL`;

  db.query(query1, [], function (error1, results1, fields1) {
  
    if (error1) throw error1;

    var query2 = `SELECT * FROM timeunits WHERE delete_date IS NULL`;
  
    db.query(query2, [], function (error2, results2, fields2) {
    
      if (error2) throw error2;

      var query3 = `SELECT * FROM taskstatus WHERE delete_date IS NULL`;

      db.query(query3, [], function (error3, results3, fields3) {
    
        if (error3) throw error3;
  
        var query4 = `SELECT * FROM taskstatus WHERE delete_date IS NULL`;
  
        var query4 = `SELECT s.id, s.schedule_name, s.datatype_id, d.name AS datatype_name, d.id AS datatype_id, s.minute, s.hour, s.day, s.month, s.week, t.name AS taskstatus_name, t.id AS taskstatus_id FROM schedules s
        LEFT JOIN datatypes d ON d.id = s.datatype_id
        LEFT JOIN taskstatus t ON t.id = s.taskstatus_id
        WHERE s.delete_date IS NULL AND d.delete_date IS NULL AND t.delete_date IS NULL`;
    
        db.query(query4, [], function (error4, results4, fields4) {
  
          if (error4) throw error4;
      
          res.render('apps/datacollector/schedules', { title: 'Express', datatypes: results1, timeunits: results2, taskstatus: results3, schedules: results4 });
...
/* GET /datacollector/privacypolicies page. */
router.get('/datacollector/privacypolicies', function(req, res, next) {
  ...
  db.query(query, [], function (error, results, fields) {
    ...
    res.render('apps/datacollector/privacypolicies', { title: 'Express', privacypolicies: results });
